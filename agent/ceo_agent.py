...
def decide(m, linkedin):
    ...
    if li_status in {'HEALTHY','ATTENTION'} and isinstance(linkedin.get('priority_1'), dict):
        ...
        if p.get('type') not in {'EVIDENCE'}:
            priorities.append({
                'rank':0,'type':'CHANNEL','owner':'Agent 8',
                'action': p.get('action') or 'Execute the highest-value LinkedIn department priority.',
                'reason': f"LinkedIn department priority: {p.get('type','UNKNOWN')}"
            })
    ...
    priorities.sort(key=lambda x: (0 if x['type']=='STABILITY' else 1 if x['type']=='BOTTLENECK' else 2 if x['type'] in {'EVIDENCE','CHANNEL'} else 3))
...
def main():
    ...
    report={
      ...
      'departments': {
          ...
          'linkedin': {
              'manager':'AGENT_8_LINKEDIN_DEPARTMENT_HEAD','health':val(s['linkedin'], 'department_health'),
              'status':val(s['linkedin'], 'department_status'),'biggest_bottleneck':val(s['linkedin'], 'biggest_bottleneck'),
              'priority_1':s['linkedin'].get('priority_1'),'ceo_escalation_required':s['linkedin'].get('ceo_escalation_required', 'UNKNOWN'),
    ...
    STATE.write_text(json.dumps({
      'last_run_at':now,'company_status':status,'north_star':report['north_star'],'priority_1':priorities[0]['action'],
      'manager_directive':report['manager_directive'],'linkedin_department_status':m['linkedin_department_status'],
      'linkedin_biggest_bottleneck':m['linkedin_biggest_bottleneck'],'linkedin_directive_id':directive.get('directive_id') if directive else 'NONE',
