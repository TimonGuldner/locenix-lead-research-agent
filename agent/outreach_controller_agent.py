...
def main() -> None:
    ...
    candidates = []
    for lead in queue:
        if lead.get('deep_qa_decision') not in eligible_decisions: continue
        if lead.get('airtable_sync_status') not in eligible_sync: continue
        if lead.get('outreach_status') not in eligible_outreach: continue
        if int(lead.get('priority_score') or 0) < min_priority: continue
        if not lead.get('airtable_record_id'): continue
    ...
    for lead in candidates[:limit]:
        ...
        try:
            airtable_update(lead['airtable_record_id'], {subject_field: subject, body_field: body, status_field: 'APPROVED'})
            results.append({'lead_id': lead.get('lead_id'), 'airtable_record_id': lead.get('airtable_record_id'), 'company_name': lead.get('company_name'), 'subject': subject, 'draft': body, 'draft_status': 'APPROVED', 'send_status': 'NOT_SENT', 'generated_at': now})
        except Exception as exc:
            errors.append(f"{lead.get('lead_id')}: {exc}")
    LATEST.parent.mkdir(parents=True, exist_ok=True)
    LATEST.write_text(json.dumps(results, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
    state = {'mode': 'DRAFT_AND_AUTO_APPROVE', 'eligible_count': len(candidates), 'stored_count': len(results), 'error_count': len(errors), 'errors': errors, 'sent_count': 0, 'last_run_at': now}
