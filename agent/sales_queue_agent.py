...
def next_action(priority,qa,vis):
    if not qa.get("email"): return "EMAIL_MISSING_RESEARCH_REQUIRED"
...
def airtable_fields(item):
    ...
    return {"Company":item.get("company_name") or item.get("lead_id") or "Unknown","Lead ID":item.get("lead_id") or "","Industry":item.get("industry") or "","City":item.get("city") or "","Website":item.get("website") or None,"Google Maps":item.get("google_maps_url") or None,"Email":item.get("email") or None,"Deep QA Decision":item.get("deep_qa_decision") or None,"Deep QA Score":item.get("deep_qa_score"),"Priority Score":item.get("priority_score"),"Sales Tier":item.get("sales_tier") or None,"Visibility Status":visibility_status,"Visibility Opportunity Score":item.get("visibility_opportunity_score"),"Primary Pitch":item.get("primary_pitch") or "","Why Now":"\n".join(str(x) for x in item.get("why_now",[]) if x),"Proof 1":item.get("proof_1") or "","Proof 2":item.get("proof_2") or "","Proof 3":item.get("proof_3") or "","Next Action":item.get("next_action") or "","Outreach Status":item.get("outreach_status") or "NOT_CONTACTED","Airtable Ready":bool(item.get("airtable_ready")),"Queue Date":datetime.now(timezone.utc).isoformat(),"Notes":f"Synced by LOCENIX Sales Queue. Email status: {item.get('email_verification_status') or 'EMAIL_MISSING'}; source: {item.get('email_source_url') or 'none'}. No outreach sent."}
...
def main():
    ...
    qa_results=load_json(QA_PATH,[]);visibility=load_json(VIS_PATH,[]);vis_by_id={x.get("lead_id"):x for x in visibility if x.get("lead_id")};decisions=set(task.get("eligible_decisions",["FINAL_A","FINAL_B"]));min_priority=int(task.get("min_priority",80));queue=[]
    for qa in qa_results:
        ...
        if vis:
            for x in vis.get("visibility_evidence",[]):
                if x and x not in proofs:proofs.append(x)
        email_ready=bool(qa.get("email") and qa.get("email_verification_status") in {"PUBLICLY_OBSERVED","VERIFIED"})
        airtable_ready=bool(qa.get("airtable_ready") and qa.get("deep_qa_decision")=="FINAL_A" and priority>=int(task.get("airtable_ready_min_priority",88)) and qa.get("primary_sales_angle_verified") and email_ready)
        queue.append({"lead_id":lead_id,"company_name":qa.get("company_name"),"industry":qa.get("industry"),"city":qa.get("city"),"website":qa.get("website"),"google_maps_url":qa.get("google_maps_url"),"email":qa.get("email"),"email_source_url":qa.get("email_source_url"),"email_verification_status":qa.get("email_verification_status") or "EMAIL_MISSING","email_found_at":qa.get("email_found_at"),"deep_qa_decision":qa.get("deep_qa_decision"),"deep_qa_score":qa.get("deep_qa_score"),"visibility_checked":bool(vis),"weak_visibility_observed":vis.get("weak_visibility_observed") if vis else None,"visibility_opportunity_score":vis.get("visibility_opportunity_score") if vis else None,"priority_score":priority,"sales_tier":"A" if priority>=90 else "B" if priority>=82 else "C","primary_pitch":qa.get("primary_sales_angle_verified"),"why_now":why_now,"proof_1":proofs[0] if len(proofs)>0 else None,"proof_2":proofs[1] if len(proofs)>1 else None,"proof_3":proofs[2] if len(proofs)>2 else None,"next_action":next_action(priority,qa,vis),"airtable_ready":airtable_ready,"airtable_sync_status":"READY_NOT_SYNCED" if airtable_ready else "EMAIL_MISSING" if not email_ready else "NOT_READY","outreach_status":"NOT_CONTACTED","queue_date":date.today().isoformat(),"queue_agent":"LOCENIX_SALES_QUEUE_AGENT_V3_EMAIL_REQUIRED","compliance_note":"Email is a hard data-quality requirement for email-pipeline handoff. Sending authorization remains separate."})
    ...
    stats=sync_airtable(queue);state={"queue_count":len(queue),"airtable_ready_count":sum(1 for x in queue if x.get("airtable_ready")),"email_ready_count":sum(1 for x in queue if x.get("email")),"email_missing_count":sum(1 for x in queue if not x.get("email")),"airtable_created":stats["created"],"airtable_updated":stats["updated"],"airtable_skipped":stats["skipped"],"airtable_errors":stats["errors"],"priority_a_count":sum(1 for x in queue if x.get("sales_tier")=="A"),"priority_b_count":sum(1 for x in queue if x.get("sales_tier")=="B"),"visibility_pending_count":sum(1 for x in queue if not x.get("visibility_checked")),"last_run_date":date.today().isoformat()}
